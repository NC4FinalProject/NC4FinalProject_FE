...
const NoticeDetail = () => {
  ...
  const getNextNotice = async () => {
    try {
      const response = await axios.get(`http://175.45.203.117:9090/notice/${nextNotice}`);
  ...
  useEffect(() => {
    const getBackNotice = async () => {
      try {
        const response = await axios.get(`http://175.45.203.117:9090/notice/${backNotice}`);
...
const handleDelete = async () => {
   try {
    ...
    const response = await axios.delete(`http://175.45.203.117:9090/notice/delete/${noticeId}`);
  ...
  useEffect(() => {
    ...
    try {
      const response = await axios.get(`http://175.45.203.117:9090/notice/likeget/${noticeId}`, {
        headers: {
  ...
  getlikedata();
    ...
    const getLikeNotice = async () => {
      try {
        const response = await axios.post(`http://175.45.203.117:9090/notice/like/${noticeId}`,null, {
          headers: {
              Authorization: `Bearer ${sessionStorage.getItem("ACCESS_TOKEN")}`,
          }
         });
  ...
  const handleDialogClose = async () => {
    try {
      await axios.put('http://175.45.203.117:9090/notice/remove');
  ...
  const handleUpload = async (file) => {
    try {
        ...
        const response = await axios.post('http://175.45.203.117:9090/notice/upload', formData, {
          headers: {
